...
import (
	...
	v1alpha1 "github.com/opentelekomcloud/provider-jet-opentelekomcloud/apis/mrs/v1alpha1"
)

// Setup adds a controller that reconciles JobV1 managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.JobV1_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.JobV1_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["opentelekomcloud_mrs_job_v1"])),
