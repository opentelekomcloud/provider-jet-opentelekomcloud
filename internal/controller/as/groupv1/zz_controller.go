...
import (
	...
	v1alpha1 "github.com/opentelekomcloud/provider-jet-opentelekomcloud/apis/as/v1alpha1"
)

// Setup adds a controller that reconciles GroupV1 managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.GroupV1_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.GroupV1_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["opentelekomcloud_as_group_v1"])),
