...
import (
	...
	v1alpha1 "github.com/enrrou/otc-provider-jet/apis/dms/v1alpha1"
)

// Setup adds a controller that reconciles QueueV1 managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.QueueV1_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.QueueV1_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["opentelekomcloud_dms_queue_v1"])),
