...
import (
	...
	v1alpha1 "github.com/opentelekomcloud/provider-jet-opentelekomcloud/apis/csbs/v1alpha1"
)

// Setup adds a controller that reconciles BackupV1 managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.BackupV1_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.BackupV1_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["opentelekomcloud_csbs_backup_v1"])),
